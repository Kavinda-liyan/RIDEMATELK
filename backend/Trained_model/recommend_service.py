...
def generate_recommendations(user_prefs, top_n=1000):
    try:
        ...
        for v in vehicles:
            ...
            gc_weight = 0.3 if user_road in ["off-road", "hilly", "mid off-road"] else 0.05
            score = fuel_priority*30 + abs(user_seating - v['Seating Capacity'])*15 - eff*2 - gc*gc_weight - road_score*5
            v['_score'] = score
        ...
        results = [{k: (None if v.get(k) is None or (isinstance(v.get(k), float) and math.isnan(v.get(k))) else v.get(k)) for k in output_cols} for v in scored_vehicles]
        return results
